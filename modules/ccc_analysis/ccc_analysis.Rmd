...
```{r read_rds, file='../../scripts/read_rds.R', warning=FALSE}
source(file.path(param$path_to_git,'scripts/read_rds.R'))
```
...
liana_res = liana_wrap(sce = sc, idents_col = "annotation", assay = "RNA", assay.type = 'logcounts', method = param$liana_methods, resource = c('Consensus'), expr_prop = 0.1, permutation = list(nperms = 500), return_all = FALSE)
#  aggregate_rank
...
liana_rank_agg = rank_aggregate(liana_res)
...
```{r fig_settings}
heatmap_number_celltypes = length(unique(sc$annotation)) * 0.3 + 0.5
...
```{r liana_res_selected, fig.height=10, warning=FALSE, message=FALSE}
