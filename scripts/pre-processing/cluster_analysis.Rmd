...
p = Seurat::DimPlot(sc, reduction="umap", group.by="seurat_clusters", pt.size=param$pt_size, split.by = "orig.ident", ncol = 2) +
  scale_color_manual(values=param$col_clusters, labels=cluster_labels) +
  AddStyle(title="Cells coloured by cluster identity", legend_position="bottom", legend_title="", xlab = "UMAP 1", ylab = "UMAP 2") +
  guides(colour = guide_legend(override.aes = list(size=3), ncol = 10))
...
for (n in seq(qc_feature)) {
  p1 = suppressMessages(Seurat::FeaturePlot(sc, features=qc_feature[n]) + 
                          AddStyle(title="Feature plot", xlab = "UMAP 1", ylab = "UMAP 2") + 
...
# Subsample to at most 50000 cells to avoid error
subsample_size = min(50000, ncol(sc))
subsample_cells = sample(1:ncol(sc), subsample_size)
sc_subsample = sc[, subsample_cells]

# Extracting distance matrix
distance_matrix = dist(Embeddings(sc_subsample[['pca']])[, 1:param$pc_n])
clusters = sc_subsample@meta.data$seurat_clusters
silhouette = cluster::silhouette(as.numeric(clusters), dist = distance_matrix)
sc_subsample@meta.data$silhouette_score = silhouette[,3]
mean_silhouette_score = mean(sc_subsample@meta.data$silhouette_score)

# Generate table
silhouette_tbl = sc_subsample@meta.data %>%
...
# Delete the object from your R environment and free up the associated memory
rm(sc_subsample)
```
...
if (markers_found) {
  ...
  if (length(levels(sc$seurat_clusters))>max_cluster_per_plot) {
    
    sec_part_tabset = TRUE
    
    nr_rows_3cols = (max_cluster_per_plot*6)/3
    fig_height_3cols = height_per_row * nr_rows_3cols
    nr_rows_3cols_part2 = ceiling((nrow(markers_top_6)-(max_cluster_per_plot*6))/3)
    fig_height_3cols_part2 = height_per_row * nr_rows_3cols_part2
  } else {
  ...
  height_per_row_variable = max(3, 2 + 0.2 * length(levels(sc$seurat_clusters)))
    
  if (length(levels(sc$seurat_clusters))>max_cluster_per_plot) {
    nr_rows_dp_2cols = (max_cluster_per_plot/2)
    fig_height_dp_2cols = height_per_row_variable * nr_rows_dp_2cols %>% min(100)
    nr_rows_dp_2cols_part2 = ceiling((length(levels(sc$seurat_clusters))-max_cluster_per_plot)/2)
    fig_height_dp_2cols_part2 = height_per_row_variable * nr_rows_dp_2cols_part2 %>% min(100)
  } else {
...
if (markers_found) {
  ...
  # Generate a subset of plot_list to print results of only max_cluster_per_plot
  # Necessary to avoid grDevices error due to too large plot size
  p_list_selected = list()
  if (length(levels(sc$seurat_clusters))>max_cluster_per_plot) {
    p_list_selected = p_list[c(1:(max_cluster_per_plot*6))]
    p_list_selected_part2 = p_list[c((max_cluster_per_plot*6+1):length(p_list))]
  } else {
    p_list_selected = p_list
  }
  
  p = patchwork::wrap_plots(p_list_selected, ncol=3) + 
...
`r ifelse(sec_part_tabset,"### Violin plots (normalised) part 2", "")` 
```{r markers_plot_violin_raw_part2, eval=sec_part_tabset, fig.height=fig_height_3cols_part2}
if (sec_part_tabset) {
  p = patchwork::wrap_plots(p_list_selected_part2, ncol=3) + 
    patchwork::plot_annotation(title="Violin plot of for normalised gene expression data, top marker genes per cluster") & theme(legend.position="none")
  p
} 
```
...
if (markers_found) {
  ...
  p_list = lapply(markers_top_10$cluster %>% sort() %>% unique(), function(cl) {
    ...
    p = suppressMessages(Seurat::DotPlot(sc, features=genes) + 
                           ...
                           guides(size=guide_legend(order=1, nrow = 1), colour = guide_colourbar(order = 2)))
    return(p)
  })
  ...
  p_list_selected = list()
  if (length(levels(sc$seurat_clusters))>max_cluster_per_plot) {
    p_list_selected = p_list[c(1:max_cluster_per_plot)]
    p_list_selected_part2 = p_list[c((max_cluster_per_plot+1):length(p_list))]
  } else {
    p_list_selected = p_list
  }
  
  p = patchwork::wrap_plots(p_list_selected, ncol=2) + patchwork::plot_annotation("Dotplot with top 10 up-regulated marker genes (scaled)")
  p
}
```

`r ifelse(sec_part_tabset,"### Dot plot (scaled) part 2", "")`  
```{r markers_plot_dot_scaled_part2, eval=sec_part_tabset, fig.height=fig_height_dp_2cols_part2}
if (sec_part_tabset) {
  p = patchwork::wrap_plots(p_list_selected_part2, ncol=2) + patchwork::plot_annotation("Dotplot with top 10 up-regulated marker genes (scaled)")
  p
} 
```
...
# Dotplots: each row contains 2 plots
# The height of dotplots is dependent on the number of clusters
height_per_row_variable = max(3, 2 + 0.2 * (sc$orig.ident %>% unique() %>% length()))

if (length(levels(sc$seurat_clusters))>max_cluster_per_plot) {
  nr_rows_dp_2cols = (max_cluster_per_plot/2)
  fig_height_degs_per_cl = height_per_row_variable * nr_rows_dp_2cols %>% min(100)
  nr_rows_dp_2cols_part2 = ceiling((length(levels(sc$seurat_clusters))-max_cluster_per_plot)/2)
  fig_height_degs_per_cl_part2 = height_per_row_variable * nr_rows_dp_2cols_part2 %>% min(100)
} else {
  nr_rows_dp_2cols = ceiling(length(levels(sc$seurat_clusters))/2)
  fig_height_degs_per_cl = height_per_row_variable * nr_rows_dp_2cols %>% min(100)
}
...
if (markers_found) {
  ...
  p = patchwork::wrap_plots(p_list_selected, ncol=2) + patchwork::plot_annotation("Dotplot per cluster with top 25 up-regulated marker genes (not scaled)") 
  p
}
```

`r ifelse(sec_part_tabset,"### Non-scaled dotplots part 2", "")`  
```{r markers_plot_dotplotpercl_unscaled_part2, eval=sec_part_tabset, fig.height=fig_height_degs_per_cl_part2}
if (sec_part_tabset) {
  p = patchwork::wrap_plots(p_list_selected_part2, ncol=2) + patchwork::plot_annotation("Dotplot per cluster with top 25 up-regulated marker genes (not scaled)") 
  p
} 
```
...
if (markers_found) {
  ...
  p = patchwork::wrap_plots(p_list_selected, ncol=2) + patchwork::plot_annotation("Dotplot per cluster with top 25 up-regulated marker genes (scaled)") 
  p
}
```

`r ifelse(sec_part_tabset,"### Scaled dotplots part 2", "")` 
```{r markers_plot_dotplotpercl_scaled_part2, eval=sec_part_tabset, fig.height=fig_height_degs_per_cl_part2}
if (sec_part_tabset) {
  p = patchwork::wrap_plots(p_list_selected_part2, ncol=2) + patchwork::plot_annotation("Dotplot per cluster with top 25 up-regulated marker genes (scaled)") 
  p
} 
```
