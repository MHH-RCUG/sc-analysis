...
#param$trajectory_cluster = c("6", "8", "9", "3", "7", "14", "11")
...
param$plot_genes = c("SOX2", "CDH5", "CD34", "GJA5", "NR2F2", "RUNX1", "PTPRC", "HBG1", "MYH6", "LUM", "TBX5", "GATA6")
#param$plot_genes = c("CDH5", "MECOM", "PTPRC", "GJA4", "GJA5", "CXCR4", "RUNX1", "IL33", "ALDH1A1", "KCNK17", "HLF", "SPINK2")
```
...
cds = SeuratWrappers::as.cell_data_set(sc)
...
cds = learn_graph(cds, close_loop = TRUE, use_partition = FALSE)
```
...
p2 = plot_cells(cds, color_cells_by = "pseudotime", label_cell_groups = FALSE, label_leaves = FALSE, label_branch_points = FALSE, graph_label_size=3, cell_size = param$pt_size) +
  theme(text = element_text(size = 12), 
  axis.line = element_line(size = 0.5, color = "grey50")) +
  NoGrid() +
...
pseudotime_values = dplyr::filter(pseudotime_values, seurat_clusters %in% param$trajectory_cluster)
...
# Split object (seurat v5)
sc_subsets = Seurat::SplitObject(sc, split.by = "orig.ident")
...
#  p = Seurat::FeatureScatter(object = subset(sc_subsets[[n]], idents = param$trajectory_cluster), feature1 = "pseudotime", feature2 = i, cols = param$col_clusters)
  p = Seurat::FeatureScatter(object = sc_subsets[[n]], feature1 = "pseudotime", feature2 = i, cols = param$col_clusters) +
    geom_smooth(method = "loess", color="black") +
    AddStyle(title = "") +
    theme(text = element_text(size = 12), 
    axis.line = element_line(size = 0.5, color = "grey50")) +
...
p = plot_cells(cds, genes = param$plot_genes, scale_to_range = TRUE, label_cell_groups=FALSE, label_leaves = FALSE, label_branch_points = FALSE, graph_label_size = 3, cell_size = param$pt_size) +
...
for (i in param$plot_genes) {
  p = FeaturePlot(sc, features = i, pt.size = param$pt_size, cols = c(param$col_bg, param$col)) +
