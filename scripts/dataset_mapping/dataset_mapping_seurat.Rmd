...
```{r fig_settings}
fig_annot_celltypes = ceiling(length(row.names(pred_celltypes))/4) * 2.5
```
...
plot_feature = gsub("_", "-", celltype_filter)

p <- FeaturePlot(sc,  reduction = "ref.umap", features = plot_feature, ncol = 4, 
...
# plot cell type prediction scores
p <- FeaturePlot(sc, reduction = param$reduction, features = plot_feature, ncol = 4,
                  cols = c(param$col_bg, param$col))
p
